...
void main() {
  ...
  group('completeTotp — pendingTotp → authenticated', () {
    ...
    test(
      ...
      // Assert
      expect(store['jwt_token'], 'jwt-full-access-new',
  ...
  group('completeEnrollment — pendingEnrollment → authenticated', () {
    ...
    test(
      ...
      // Assert
      expect(provider.backupCodes, expectedCodes,
  ...
  group('logout — authenticated → unauthenticated', () {
    ...
    test(
      ...
      expect(provider.backupCodes, isNull,
  ...
  group('transiciones inválidas — deben ser no-ops', () {
    ...
    test(
      ...
      // Assert
      expect(provider.state, AuthState.unauthenticated,
          reason: 'completeEnrollment es no-op si no estamos en pendingEnrollment');
    });
  ...
  group('edge cases', () {
    ...
    test(
      ...
      // Assert
      expect(provider.state, AuthState.unauthenticated,
          reason: 'Sin scope claim no podemos determinar estado, sesión inválida');
    });

    test(
        'login exitoso expone username en provider',
        () async {
      // Arrange
      fakeApi.loginResponse = {
        'access_token': 'jwt-full',
    ...
    test(
        'notifyListeners se llama tras cada transición de estado',
        () async {
